...
export function RecordPaymentSheet({ open, onOpenChange, defaultTenantId }: RecordPaymentSheetProps) {
  ...
  const onSubmit = async (values: RecordFormValues) => {
    if (values.recordType === 'deposit') {
      ...
      const forMonth = values.paymentDate.slice(0, 7) + '-01'
      rentMutation.mutate({
        tenantId: values.tenantId,
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="max-h-[85vh] overflow-y-auto rounded-t-xl">
        <SheetHeader>
          <SheetTitle>{view === 'success' ? (successInfo?.kind === 'deposit' ? 'Deposit Recorded' : 'Payment Recorded') : 'Record Payment'}</SheetTitle>
        </SheetHeader>
